...
test('logging in as a venue manager', async ({ page }) => {
  ...
  if (!username || !password) {
    throw new Error(
      'Please provide .env PLAYWRIGHT_USERNAME and PLAYWRIGHT_PASSWORD'
    );
  }
  ...
  await expect(
    page.getByText('Welcome back venue_manager! 👋').first()
  ).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Holidaze' })).toBeVisible();
  await expect(
    page.getByRole('heading', { name: 'Your Gateway to Unforgettable' })
  ).toBeVisible();
  await page.getByRole('banner').getByRole('button').click();
  await page.getByRole('menuitem', { name: 'Profile' }).click();
  await expect(
    page.getByRole('heading', { name: 'venue_manager' })
  ).toBeVisible();
});
...
test('Registration page', async ({ page }) => {
  ...
  await test.step('assert that the form is visible', async () => {
    ...
    await expect(
      page.getByRole('button', { name: 'Register ☀️' })
    ).toBeVisible();
  ...
  await test.step('form gives validation errors', async () => {
    ...
    await page.getByPlaceholder('email').fill('test@test.com');
    await expect(
      page.getByText('Only @stud.noroff.no emails are allowed')
    ).toBeVisible();
  });
});
